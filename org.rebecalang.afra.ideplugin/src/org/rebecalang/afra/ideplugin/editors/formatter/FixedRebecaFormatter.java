...
/**
 * Robust token-based formatter for Rebeca (.rebeca) files
...
public class FixedRebecaFormatter implements IAfraFormatter {
    ...
    private static final Set<String> KEYWORDS = new HashSet<>(Arrays.asList(
        "reactiveclass", "if", "else", "msgsrv", "boolean", "byte", "short", "int", 
        "true", "false", "knownrebecs", "statevars", "main", "self", "sender",
        "externalclass", "sends", "of", "globalvariables", "bitint", "after", "deadline", "delay"
    ));
    ...
    // Unary operators
    private static final Set<String> UNARY_OPERATORS = new HashSet<>(Arrays.asList("!", "-", "+"));
    ...
    private String formatContent(String content) {
        ...
        // First pass: handle comments and preserve them
        List<Token> tokens = tokenize(content);
        
        // Second pass: format tokens
                        ...
                        if (prev != null && prev.type == TokenType.IDENTIFIER) {
                            // No space for function calls
                                  ...
                                  (prev.type == TokenType.KEYWORD && 
                                   ("if".equals(prev.value) || "else".equals(prev.value)))) {
                            result.append(" ");
                        }
                        result.append(token.value);
                    } else { // ")"
    ...
    private boolean needsSpaceBefore(Token token, Token prev) {
        ...
        if (prev.type == TokenType.IDENTIFIER && token.type == TokenType.IDENTIFIER) {
            return true;
        }
